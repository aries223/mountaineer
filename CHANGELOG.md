...
- Log injection vulnerability (S5145) in `preferences.py`; user-controlled preference values now sanitised via `_sanitise_for_log` and suppressed with `# NOSONAR`
...
- Taskbar icon not displaying correctly on Wayland; added `app.setDesktopFileName("mountaineer")` to `src/main.py`
- Taskbar icon not displaying correctly on X11; added `StartupWMClass=mountaineer` to `src/ui/logo/mountaineer.desktop`
- RPM spec corrected: lowercase package name, proper `Requires` entries, complete `%install` section, correct SVG icon path
